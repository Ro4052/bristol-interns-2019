...
describe('if given no cards', () => {
    it("doesn't display any cards", () => {
        ...
        expect(wrapper.find({ 'data-cy': 'played-cards' }).children().length).toEqual(0);
    });
});
...
describe('if given one card', () => {
    it("displays one card", () => {
        ...
        expect(wrapper.find({ 'data-cy': 'played-cards' }).children().length).toEqual(1);
    });
});
...
describe('if given two cards', () => {
    it("displays two cards", () => {
        ...
        expect(wrapper.find({ 'data-cy': 'played-cards' }).children().length).toEqual(2);
    });
});
