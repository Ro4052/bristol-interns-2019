    ...
    render() {
        return (
            <div className={styles.cardsContainer} id="my-cards">
                <ul>
                    {this.props.myCards.map((cardId) => <img id={"card-" + cardId} alt={"card-" + cardId} className={this.getCardClass()} key={cardId} src={`./cards/card (${cardId}).jpg`} onClick={this.handleClickCard} />)}
                </ul>
            </div>
        );
    }
